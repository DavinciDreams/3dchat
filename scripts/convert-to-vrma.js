#!/usr/bin/env node
/**
 * FBX to VRMA Batch Converter
 *
 * Converts all FBX files in animations-raw/ to VRMA format in public/animations/
 * Uses the fbx2vrma-converter tool.
 */
...
const FBX2GLTF_PATH = path.join(__dirname, '..', 'tools', 'fbx2vrma-converter', 'FBX2glTF-darwin-x64');
...
async function main() {
  console.log('🎬 FBX to VRMA Batch Converter\n');

  // Check if input directory exists
  if (!fs.existsSync(INPUT_DIR)) {
    console.log(`📁 Creating input directory: ${INPUT_DIR}`);
    fs.mkdirSync(INPUT_DIR, { recursive: true });
    console.log('\n⚠️  No FBX files found. Please add FBX files to animations-raw/ directory.');
    console.log('   Run: node scripts/download-mixamo.js --help');
    return;
  }
  ...
  if (!fs.existsSync(CONVERTER_PATH)) {
    ...
    console.error('   Run: git clone https://github.com/tk256ailab/fbx2vrma-converter.git tools/fbx2vrma-converter');
    process.exit(1);
  }
  ...
  if (!fs.existsSync(FBX2GLTF_PATH)) {
    ...
    console.error('   Run: cd tools/fbx2vrma-converter && ./setup.sh');
    process.exit(1);
  }
  ...
  if (fbxFiles.length === 0) {
    ...
    console.log('   Download animations from Mixamo first:');
    console.log('   node scripts/download-mixamo.js --email YOUR_EMAIL --password YOUR_PASSWORD');
    return;
  }
  ...
  if (converted.length > 0) {
    ...
    converted.forEach(name => {
      console.log(`  '${name}': '/animations/${name}.vrma',`);
    });
    console.log('};');
  }
}
