...
import { speakChunk, stopStreamingSpeech, stopAudio } from '../services/speechSynthesisService';
...
const ChatInterface = (): JSX.Element => {
  ...
  // Track active animation queue timeouts for cancellation
  const activeQueueTimeoutsRef = useRef<NodeJS.Timeout[]>([]);
  // Track if judgment has been made for current response
  ...
  // Cancel previous animation queues to prevent multiple independent queues
  ...
  const handleMessage = async (content: string) => {
    ...
    try {
      // Reset judgment flag for new message
      // This ensures we only judge animations once per response
      hasMadeJudgmentRef.current = false;

      // Cancel any previous animation queues from previous messages
      ...
      await streamAIResponse(content, {
        onChunk: async (chunk) => {
          if (chunk.isComplete) {
            // Make animation judgment when streaming completes
            ...
            if (!hasMadeJudgmentRef.current && fullText.length > 20) {
              ...
              // Cancel any previous animation queue timeouts before final judgment
              cancelActiveQueueTimeouts();
              ...
              if (animationJudgment.animations.length > 0) {
                ...
                // Process animation queue with timeout tracking for cancellation
      ...
      >
        ...
        <div className="flex items-center gap-2 px-2 py-1.5 border-b border-white/5 bg-gray-800/30">
          ...
          >
            ...
            {[...new Set(AVAILABLE_ANIMATIONS)].map((anim) => (
